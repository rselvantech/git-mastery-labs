...
"You have 10 files across your project with identical contents. git add all 10. How many blob objects are stored in .git/objects/?","One. Git is content-addressed — same content produces the same SHA-1 hash, which maps to the same stored object. Deduplication is automatic.","demo01 internals content-addressed deduplication"
...
"What three situations cause Git to open the editor defined in core.editor?","1) git commit without -m flag — write commit message. 2) git rebase -i — edit the list of commits. 3) git merge — edit the auto-generated merge commit message.","demo01 config core.editor usage"
